...
@celery_app.task(
    name="tasks.process_idle_conversations",
    queue=QUEUEEnum.ANALYSIS.value,
)
def process_idle_conversations():
    """
    Chunked dispatcher — pages through idle conversations in batches of
    BATCH_SIZE. Each page spawns child tasks then immediately moves to
    the next page. Never loads all rows into memory at once.
    """
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)

    try:
        total_spawned = loop.run_until_complete(_dispatch_in_batches())
    finally:
        loop.close()
        asyncio.set_event_loop(None)

    logger.info(f"[idle-cron] Done. Total spawned: {total_spawned}")
    return {"spawned": total_spawned}
